...
func resolveModelAndProvider(ctx *fasthttp.RequestCtx, config *lib.Config, model string) (schemas.ModelProvider, string, error) {
	...
	if provider == "" {
		mc := config.ModelCatalog
		if mc == nil {
			return "", "", fmt.Errorf("provider is required in model field (format: provider/model) and model catalog is not available")
		}
		providers := mc.GetProvidersForModel(modelName)
		if len(providers) == 0 {
